...
day05-chat_prompt = ChatPromptTemplate(
    [
...
# 参数个数形式传递
#prompt_value = day05-chat_prompt.format_messages(role="python开发工程师", question="冒泡排序怎么写")

# 字典解包形式传递
prompt_value = chat_prompt.format_messages(**{"role": "python开发工程师", "question": "堆排序怎么写"})
# 打印格式化后的提示消息
print(prompt_value) # 消息对象的集合
print(type(prompt_value)) #<class 'list'>
...
# format形式 vs day03-invoke 形式（直接传入的是字典类型）
prompt_value2 = chat_prompt.invoke({"role": "python开发工程师", "question": "堆排序怎么写"})
# 输出生成的提示内容
print(prompt_value2) #messages = [消息集合]
print(type(prompt_value2)) #<class 'langchain_core.prompt_values.ChatPromptValue'>
# ChatPromptValue 是包着列表的壳
# 要拿列表，用 .messages
...
result = llm.invoke(prompt_value2)
print(result)
